...
function SetupPage() {
      ...
      try {
        ...
        if (parsedData.semesterData) {
          ...
          if (parsedData.semesterData.holidays) {
            const holidayDates = parsedData.semesterData.holidays.map(holiday => new Date(holiday + 'T00:00:00'));
  ...
  const handleHolidaySelect = (dates) => {
    // Check if dates is defined before mapping
    if(dates){
        setSelectedHolidays(dates);
        const holidayStrings = dates.map(date => format(date, 'yyyy-MM-dd'));
        setSemesterData(prev => ({
            ...prev,
  ...
  // THIS IS THE ONLY CORRECTED PART
  const removeHoliday = (holidayToRemove) => {
    const updatedHolidays = selectedHolidays.filter(holiday => 
      format(holiday, 'yyyy-MM-dd') !== holidayToRemove
    );
    setSelectedHolidays(updatedHolidays);
    setSemesterData(prev => ({
      ...prev,
      holidays: updatedHolidays.map(date => format(date, 'yyyy-MM-dd')),
    }));
  };
  ...
  const handleClearForm = () => {
    const confirmed = window.confirm('Are you sure you want to reset all setup data?');
    if (confirmed) {
      setSchedules([]);
      setSemesterData({
        startDate: '',
    ...
    <div className="space-y-8 p-4 md:p-8">
                    ...
                    <tr key={schedule.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input 
                          ...
                          onKeyDown={(e) => handleTimeKeyPress(e, schedule.id)}
                  ...
                  {selectedHolidays
                    ...
                    .map((holiday, index) => (
                      <div key={index} className="flex items-center justify-between bg-gray-50 px-3 py-2 rounded-md">
                        ...
                        <button 
                          onClick={() => removeHoliday(format(holiday, 'yyyy-MM-dd'))} 
      ...
      <div className="text-center py-6 space-x-4">
        <button 
          ...
          className="bg-gray-600 hover:bg-gray-700 text-white px-8 py-3 rounded-md text-lg font-medium"
        >
          Clear Form
        </button>
        <button 
